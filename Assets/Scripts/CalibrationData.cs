...
public class CalibrationData
{
    ...
    private bool IsQuaternionInvalid(Quaternion q) { bool check = q.x == 0f; check &= q.y == 0; check &= q.z == 0; check &= q.w == 0; return check; }
    public void Tick(Quaternion newTarget, float speed)
    {
        if (IsQuaternionInvalid(newTarget) || IsQuaternionInvalid(targetRotation))
        {
            return;
        } else {
